...
```{r}
mpa_2021_summarized <- read_csv(here("raw-data", "field_data_2020_2021.csv"), show_col_types = FALSE) %>% 
   filter(year == 2021, 
          site == 2) %>% 
  select(year, spp, month, predawn, midday, type, date) %>% 
  drop_na(year) %>% 
 ...
 # select(-month_char) %>% 
  mutate(mpa_se = plotrix::std.error(mpa), 
         mpa_sd = sd(mpa, na.rm = T),
         mpa_mean = mean(mpa)) %>% 
  select(-mpa) %>% 
  distinct()

write_csv(mpa_2021_summarized, here("processed-data", "xpp_2021_tree2shrub.csv"))
```

2021:

```{r}
mpa_2021_summarized_spp <- read_csv(here("raw-data", "field_data_2020_2021.csv"), show_col_types = FALSE) %>% 
   filter(year == 2021, 
          site == 2) %>% 
  select(year, spp, month, predawn, midday, type, date) %>% 
  drop_na(year) %>% 
  ...
  pivot_longer(cols = c(midday, predawn), names_to = "timing", values_to = "mpa") %>% 
  drop_na(mpa) %>% 
  group_by(month_split, spp, timing) %>% 
 # select(-month_char) %>% 
  mutate(mpa_se = plotrix::std.error(mpa), 
         mpa_sd = sd(mpa, na.rm = T),
         mpa_mean = mean(mpa)) %>% 
...
write_csv(mpa_2021_summarized_spp, here("processed-data", "xpp_2021_spp.csv"))
```
...
```{r}
mpa_2020_summarized <-read_csv(here("raw-data", "field.summer.2020.csv"), show_col_types = FALSE) %>%
  clean_names()  %>%
 ...
 # select(-month_char) %>% 
  mutate(se_mpa = plotrix::std.error(mpa), 
         ...
         mean_mpa = mean(mpa, na.rm = T)) %>% 
  filter(!spp %in% c("QUKE", "PIJE")) %>% 
  select(-site)

write_csv(mpa_2020_summarized, here("processed-data", "xpp_2020_all.csv"))
```
